...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (req.query.name != null) {
      urll = 'http://localhost:6000/adminapi/users/search_user?text=' + req.query.name;
    } else {
      urll = 'http://localhost:6000/adminapi/users/all';
    }
...
router.post('/', async (req, res) => {
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhZG1pbklEIjoyLCJlbWFpbCI6ImNhY2EiLCJoYXNoZWRwd2QiOiIkMmIkMTAkRnNWN0VzdlB0TUpINjhXODBlQUtSLjRCdDdEZkU0ZnJCVU1SaGpCWWRhWjBKeFQ3L0VPNnkiLCJpYXQiOjE2NDA1Mjc0NDUsImV4cCI6MTY3MjA2MzQ0NX0.DkrijX-tOaIi1zy1DMEJUZIC0NJbMhdfO3dokSQhB9I';
  // console.log(token);
  const urll = 'http://localhost:6000/adminapi/users/add';
  const headerss =
  {
    'authorization': 'Bearer ' + token
  };
...
router.get('/:id', async (req, res) => {
  try {
    const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhZG1pbklEIjoyLCJlbWFpbCI6ImNhY2EiLCJoYXNoZWRwd2QiOiIkMmIkMTAkRnNWN0VzdlB0TUpINjhXODBlQUtSLjRCdDdEZkU0ZnJCVU1SaGpCWWRhWjBKeFQ3L0VPNnkiLCJpYXQiOjE2NDA1Mjc0NDUsImV4cCI6MTY3MjA2MzQ0NX0.DkrijX-tOaIi1zy1DMEJUZIC0NJbMhdfO3dokSQhB9I';
    // console.log(token);
    const urll = 'http://localhost:6000/adminapi/users/' + req.params.id;
    const headerss =
    {
      'authorization': 'Bearer ' + token
    };
